"""
    pp_sharp(primalmesh::Primalmeshstruct, dualmesh::Dualmeshstruct, edgeform::Array{Float64,1}, Nv::Int64, Ne::Int64)

primal-primal sharp operator to map primal 1-forms to vector fields on primal vertices
"""
function pp_sharp(primalmesh::Primalmeshstruct, dualmesh::Dualmeshstruct, edgeform::Array{Float64,1}, Nv::Int64, Ne::Int64)
    ...
    tempvecs = [zeros(3) for _ in 1:nthreads()]
    Threads.@threads for i = 1:Nv
        nodeid = nodekeylist[i]
        tempvecs[threadid()] = [0.0, 0.0, 0.0]
        ...
        for edgeid in edgelist
            ...
            tempvecs[threadid()] += sgn * edgeform[edge_ind] * w
        end
        pvec[i, :] = tempvecs[threadid()]
    end
...
"""
    pp_sharp_least_sq(primalmesh::Primalmeshstruct, edgeform::Array{Float64,1}, edgekeylist::Array{SArray{Tuple{2},Int64,1,2},1}, 
                           nodekeylist::Array{Any,1})

Least square approximation of the primal-primal sharp operator
"""
function pp_sharp_least_sq(primalmesh::Primalmeshstruct, edgeform::Array{Float64,1}, edgekeylist::Array{SArray{Tuple{2},Int64,1,2},1},
    nodekeylist::Array{Any,1})
    Nv::Int64, Ne::Int64
    pvec = Array{Float64,2}(undef, Nv, 3)
    ...
    for i = 1:Nv
        ...
        for ne = 1:nedge
            edge = edgelist[ne]
            edgeid = edge[1]
            v1 = primalmesh.nodedict[edgeid[1]].coords
            v2 = primalmesh.nodedict[edgeid[2]].coords
...
"""
    pd_sharp_surface3(primalmesh::Primalmeshstruct, edgeform::Array{Float64,1}, ebound3::Array{Any,1}, fbound3::Array{Any,1})
...
"""
    pp_sharp_least_sq_openbc(primalmesh::Primalmeshstruct, edgeform::Array{Float64,1}, edgekeylist::Array{SArray{Tuple{2},Int64,1,2},1}, 
